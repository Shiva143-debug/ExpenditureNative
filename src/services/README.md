...
- `getExpenseItemsByCategory(userId, category)` - Get expense items for a specific category and user
- `addExpenseItem(userId, category, expenseItem)` - Add a new expense item
...
- `getIncomeSources(userId)` - Get default sources for a user
- `addSource(userId, source, amount, date)` - Add a new income source
- `addIncomeSource(userId, sourceName)` - Add a new default source
...
const handleSubmit = async () => {
  try {
    const expenseData = {
      ...
      expense_item: selectedExpenseItem,
      cost: amount,

...
import { getIncomeSources, addIncomeSource, deleteIncomeSource, updateIncomeSource } from '../../services/apiService';
...
const SourcesScreen = () => {
  ...
  //   try {
  //     setDialogLoading(true);
  //     await updateIncomeSource(editingSource.id, id);
  ...
  const handleUpdateSource = async () => {
    ...
    try {
      ...
      await updateIncomeSource(
        ...
        id,                 // user id
        { sourceName: newSource.trim() } // body
      );
...
const styles = StyleSheet.create({
  ...
  input: {
    borderWidth: 1,
    borderColor: 'rgba(165, 162, 162, 0.3)',
    borderRadius: 8,
    padding: 12,
    fontSize: 16,
    marginBottom: 20,
